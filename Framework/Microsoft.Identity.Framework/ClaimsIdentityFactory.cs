...
namespace Microsoft.Identity
{
    ...
    internal class ClaimsIdentityFactory<TUser> : ClaimsIdentityFactory<TUser, string> where TUser : class, IUser<string>
    {
    }
    ...
    internal class ClaimsIdentityFactory<TUser, TKey> : IClaimsIdentityFactory<TUser, TKey>
        ...
        where TKey : IEquatable<TKey>
    {
            ...
            string authenticationType)
        {
            ...
            {
                IEnumerable<KeyValuePair<TKey, string>> roles = await manager.GetUserRolesAsync(user.Id).WithCurrentCulture();
                foreach (KeyValuePair<TKey, string> role in roles)
                {
                    id.AddClaim(new Claim(RoleClaimType, role.Value, ClaimValueTypes.String));
                }
            }
